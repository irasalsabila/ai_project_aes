...
data_path = '/home/iman.alsikaiti/workspace/AI_project/dataset/asap++_data.csv'
X = pd.read_csv(data_path)
X = X.dropna(subset=['essay', 'content'])
print(X['essay'])

data = pd.DataFrame()
data['essay'] = X['essay']

y = np.round(X['content']) # content scores are between 0-6
...
def get_model():
    ...
    # Target range is 0-6, we need  output classes
    output = Dense(7, activation='softmax', name='score')(x)
...
for traincv, testcv in cv.split(X):
    ...
    if count == 5:
        lstm_model.save('./overall_content_lstm_bert.h5')
